 ...
 * action's family — `projects.write` for a section or a Home shortcut placement, `tasks.write` for
...
export const undoTools: readonly WorkManagerTool[] = [
  defineTool({
    ...
    description:
      'Read this connection’s own Undo/Redo history for a project: { projectId, historyId, revision, undo, redo, blockedBy }. undo and redo name the next action in each direction — { actionId, operation, label, expiresAt } — or null at either end of the stack. operation is one of section.add, section.move, section.update, section.remove, section.restore, task.add, task.update, task.archive, task.restore, reflection.add, reflection.update, reflection.archive, reflection.restore, shortcut.add, shortcut.update, shortcut.move or shortcut.remove. historyId is null until this connection makes an undoable write in the project. Pass historyId, the action id and revision to undo_operation or redo_operation. This read needs projects.read; a write-only connection can skip it entirely, because every write receipt and every transition result carries what the next call needs. Another connection’s or a person’s history is never visible here.',
  ...
  defineTool({
    name: 'undo_operation',
    description:
      `Undo the next operation in this connection’s history. Every undoable write returns an operation receipt { historyId, actionId, revision, … }: the five section tools including restore_section, add_section_shortcut and remove_section_shortcut, and create_task, update_task, complete_task, archive_task, restore_task, add_reflection, archive_reflection and restore_reflection. Section duplication and shortcut resizing, collapsing and reordering are HTTP-only here and record the same way. Pass historyId, actionId and expectedRevision (the history's current revision: the receipt's, or get_operation_history's when anything happened since). Undo deletes a row or section the create made — including a task list or reflections container that create had to add — restores changed fields without touching fields somebody else changed, puts a moved task and its subtree back, reopens a completed task, or reverses an archive with its exact cascade markers, re-archives exactly what a section restore revived, or removes a Home shortcut placement without touching the section it points at. Only the newest applied action can be undone; undo them in order. The grant needed is the one for that action's family: projects.write for a section or a shortcut, tasks.write for a task, reflections.write for a reflection. The result is { direction, actionId, result, summary } with the refreshed history. ${REFUSALS} restore_section and restore_task remain the durable way to recover archived content once Undo is no longer possible.`,
