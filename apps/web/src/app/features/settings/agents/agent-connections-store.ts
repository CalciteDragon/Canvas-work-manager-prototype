...
export class AgentConnectionsStore {
  ...
  async load() { this.loadingState.set(true); this.errorState.set(null); try { this.itemsState.set(await this.gateway.agents.list()); } catch (e) { this.errorState.set(messageOf(e)); } finally { this.loadingState.set(false); } }
  ...
  private async write(id: AgentConnectionId, operation: () => Promise<AgentConnection>): Promise<boolean> {
    ...
    try {
      ...
      return true;
    } catch (e) { this.errorState.set(messageOf(e)); return false; } finally { this.busyState.set(null); }
  }
}
