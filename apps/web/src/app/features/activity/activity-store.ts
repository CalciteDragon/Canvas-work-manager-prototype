...
export class ActivityStore {
  ...
  async load(projectId: ProjectId, limit = 20) { const generation = ++this.generation; this.loadingState.set(true); this.errorState.set(null); const settled = this.pendingTasks.add(); try { const entries = await this.gateway.activity.list({ projectId, limit }); if (generation === this.generation) this.itemsState.set(entries); } catch (e) { if (generation === this.generation) { this.itemsState.set([]); this.errorState.set(messageOf(e)); } } finally { settled(); if (generation === this.generation) this.loadingState.set(false); } }
}
