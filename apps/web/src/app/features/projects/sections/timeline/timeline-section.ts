...
export class TimelineSection { readonly section = input.required<ProjectSection>(); readonly onConfigChange = input.required<(config: SectionConfig) => void>(); readonly onProjectDataChange = input.required<() => void>(); readonly store = inject(TimelineStore); constructor() { effect(() => void this.store.load(this.section().projectId)); } label(kind: string) { return kind === 'sub-project' ? 'Sub-project' : kind[0]!.toUpperCase() + kind.slice(1); } }
