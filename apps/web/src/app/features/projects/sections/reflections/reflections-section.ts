...
export class ReflectionsSection {
  ...
  async create(event: SubmitEvent, body: HTMLTextAreaElement, title: HTMLInputElement, prompt: HTMLSelectElement) { event.preventDefault(); if (this.readOnly()) return; if (await this.store.create(body.value, title.value, prompt.value)) { body.value = ''; title.value = ''; prompt.value = ''; this.onProjectDataChange()(); } }
}
