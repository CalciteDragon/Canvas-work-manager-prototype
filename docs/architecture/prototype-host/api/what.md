...
| Sections | `GET /api/projects/:projectId/sections`, `POST /api/projects/:projectId/sections`, `PATCH /api/sections/:id`, `DELETE /api/sections/:id` (200 with an Undo receipt; exact-owner repeat can return 409 receipt details), `POST /api/sections/:id/move`, `/duplicate`, `/restore` |
