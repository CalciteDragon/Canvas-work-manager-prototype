...
4. Edit Layout Mode enables CDK drag-drop, size presets, settings, duplicate, remove and
   Quick Add; every write goes to the host and paints optimistically behind
   `pendingWrites`. A refused removal with `section_not_empty` details opens the dialog.
...
| `ProjectCanvas` | component | One page's canvas | [API](../../../api/components/ProjectCanvas.html) |
...
pnpm storybook                          # ProjectCanvas, ProjectPageNavigation, frame, pages, shortcuts, archive list
pnpm e2e                                # web.spec, todos.spec, archive.spec, reflections.spec
```
