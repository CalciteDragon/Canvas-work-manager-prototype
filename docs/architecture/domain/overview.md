...
- **Undo.** Explicit section add, move and settings writes, as well as removal, record one
  scoped, expiring inverse through `UndoRecorder` in their caller-owned unit and return a
  sequence-ordered receipt. `UndoService` executes the typed family once for the exact actor,
  with field-aware conflicts, combined-neighbour placement and typed refusals instead of
  overwriting later writes. Automatic row-container creation remains unrecorded.
