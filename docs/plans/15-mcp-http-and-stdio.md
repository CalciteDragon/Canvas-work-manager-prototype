...
| `docs/mcp-setup.md` | Document localhost HTTP and stdio configuration, fixture tokens, environment variables, client examples, troubleshooting for Claude Desktop and Cursor, and the safe rule that a mutation-capable stdio process must not run concurrently with the HTTP/UI host's independently cached JSON store. |
