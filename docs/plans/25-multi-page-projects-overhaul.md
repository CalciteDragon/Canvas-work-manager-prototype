...
- Fix 25.4's canvas defect this slice's own canvas work sits on top of: `project-page-store.ts` reads `shortcuts.list` on **every** canvas load, and `SectionShortcutService` refuses any page that is not a root's Home, so every sub-project canvas shows a red section error on a plain visit (`.prototype/notes.json`, `note-2026-09-06-006`, observed in Slice 25.5's browser pass). Guard the read on the page kind the canvas already knows — the shell passes `shortcutsAllowed` — rather than swallowing the refusal or relaxing the domain rule, which is correct as written.
